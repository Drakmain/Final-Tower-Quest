...
extern void caracteristiques(game_t *game, character_t *character, SDL_Texture *texture_render_menu_ig, SDL_bool *echap_relache)
{
    ...
    while ((*game->program_launch) && carac_bool)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (keyState[SDL_SCANCODE_DOWN] && event.type == SDL_KEYDOWN)
            {
                ...
                if (character_points_dispo == 0)
                {
                    ...
                    if (selection == 2 && points_uti_intell == 0)
                        selection++;
                    if (selection == 3 && points_uti_agi == 0)
                        selection++;
                    if (selection == 4 && points_uti_def == 0)
                        selection++;
                }
            }
            ...
            if (character_points_dispo == character->points_dispo)
                select_ret_val = SDL_TRUE;
