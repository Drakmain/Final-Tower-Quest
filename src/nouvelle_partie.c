...
extern
void nouvelle_partie_f(SDL_Renderer * render, int windowwidth, int windowheight, SDL_bool * program_launch){
    ...
    surf_choisir_empla = TTF_RenderText_Blended(police, "CHOISISSEZ EMPLACEMENT", blanc);
    ...
    if(surf_cadre == NULL)
    {
        SDL_ExitWithError("probleme chargement image cadre choix emplacement");
    }
    ...
    while (nouv_part_bool && *program_launch)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (keyState[SDL_SCANCODE_RETURN] && event.type == SDL_KEYDOWN)
            {
                ...
                if(selection == 0)
                {
                    creation_perso(render, windowwidth, windowheight, program_launch);
                }
                if(selection == 1)
                {
                    creation_perso(render, windowwidth, windowheight, program_launch);
