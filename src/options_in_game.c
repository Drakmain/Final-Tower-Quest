...
void options_in_game(game_t * game, SDL_Texture * texture_render_menu_ig, SDL_bool *echap_relache){
    ...
    SDL_Surface* surf_options = NULL, *surf_retour = NULL, *surf_cadre = NULL, *surf_opt_resolution = NULL, *surf_choix_resolution = NULL, *surf_opt_fullscreen = NULL, *surf_choix_fullscreen = NULL, *surf_fleche = NULL;

    const Uint8 *keyState = SDL_GetKeyboardState(NULL);

    SDL_bool options_in_game_bool = SDL_TRUE;
    SDL_Event event;
    ...
    pos_choix_resolution.y = (*game->WINDOWHEIGHT)/6.75 + (*game->WINDOWHEIGHT)*12/675 + (*game->WINDOWHEIGHT)/13.5 + (*game->WINDOWHEIGHT)*35/720;
    pos_choix_resolution.w = (*game->WINDOWWIDTH)*120/1200;
    ...
    pos_choix_fullscreen.y = pos_choix_resolution.y + pos_choix_resolution.h + (*game->WINDOWHEIGHT)*20/675;;
    pos_choix_fullscreen.w = (*game->WINDOWWIDTH)*45/1200;
    ...
    if(surf_cadre == NULL)
    {
        SDL_ExitWithError("probleme chargement cadre option menu_in_game");
    }

    SDL_Texture* cadre = SDL_CreateTextureFromSurface(game->render, surf_cadre);
    if(cadre == NULL)
    {
        SDL_ExitWithError("probleme texture cadre option menu_in_game");
    }
    ...
    while (options_in_game_bool && *game->program_launch)
    {
        ...
        while (SDL_PollEvent(&event))
        {
            ...
            if (event.type == SDL_QUIT)
            {
                *game->program_launch = SDL_FALSE;
                options_in_game_bool = SDL_FALSE;
            }
            ...
            if(selection == 1){
                ...
                if (keyState[SDL_SCANCODE_RIGHT] && event.type == SDL_KEYDOWN)
                {
                    if(*game->etat_fullscreen == SDL_FALSE)*game->etat_fullscreen = SDL_TRUE;
                }
            ...
            pos_options.y = pos_cadre.y + rect_fond_cadre.y + (*game->WINDOWHEIGHT)*15/720;
            ...
            pos_opt_resolution.y = pos_options.y + pos_options.h + (*game->WINDOWWIDTH)*20/1200;
            pos_opt_resolution.w = (*game->WINDOWWIDTH)*150/1200;
            pos_opt_resolution.h = (*game->WINDOWHEIGHT)/13.5;

            pos_choix_resolution.x = (*game->WINDOWWIDTH)/6 + (*game->WINDOWWIDTH)*500/1200;
            pos_choix_resolution.y = pos_opt_resolution.y;
            pos_choix_resolution.w = (*game->WINDOWWIDTH)*120/1200;
            pos_choix_resolution.h = (*game->WINDOWHEIGHT)/13.5;

            pos_opt_fullscreen.x = (*game->WINDOWWIDTH)/6 + (*game->WINDOWWIDTH)*50/1200;
            pos_opt_fullscreen.y = pos_opt_resolution.y + pos_opt_resolution.h + (*game->WINDOWHEIGHT)*20/675;
            pos_opt_fullscreen.w = (*game->WINDOWWIDTH)*165/1200;
            pos_opt_fullscreen.h = (*game->WINDOWHEIGHT)/13.5;

            pos_choix_fullscreen.x = (*game->WINDOWWIDTH)/6 + (*game->WINDOWWIDTH)*500/1200;
            pos_choix_fullscreen.y = pos_opt_fullscreen.y;
            pos_choix_fullscreen.w = (*game->WINDOWWIDTH)*45/1200;
            pos_choix_fullscreen.h = (*game->WINDOWHEIGHT)/13.5;

            pos_retour.w = (*game->WINDOWWIDTH)/(1200/129);
            pos_retour.h = (*game->WINDOWHEIGHT)/13.5;
            pos_retour.x = pos_opt_resolution.x;
