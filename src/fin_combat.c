 ...
 */
 ...
 extern
 void fin_combat(game_t *game, character_t *character, map_t *map, int nb_enemie_combat, int rand_enemie[])
{
    srand(time(NULL));
    ...
    if(nb_enemie_combat == 1)
    {
        ...
        affichage_message(game, "Vous avez gagne %d xp.", nb_total_xp);
        ...
        if(character->xp + character->tab_xp_max[character->lvl - 1] > character->tab_xp_max[character->lvl])
        {
            ...
            affichage_message(game, "Vous avez gagne un niveau.", -1);
            character->lvl++;
            affichage_message(game, "Vous avez obtenu 3 points de compétence.", -1);
        ...
        if(strcmp(map->name,"floor_1") == 0)
        {
            ...
            {
                affichage_message(game, "Vous avez obtenue une potion de puissance.", -1);
                character->nb_popo_puissance++;
            }
            else if(random_drop_potions < 8)
            {
                affichage_message(game, "Vous avez obtenue une potion d'agilite.", -1);
                character->nb_popo_agi++;
            }
            else if(random_drop_potions < 14)
            {
                affichage_message(game, "Vous avez obtenue une petite potion de soin.", -1);
                character->nb_petite_popo_soin++;
            }
            else if(random_drop_potions < 20)
            {
                affichage_message(game, "Vous avez obtenue une petite potion de mana.", -1);
        ...
        else if(strcmp(map->name,"floor_2") == 0)
        {
            ...
            {
                affichage_message(game, "Vous avez obtenue une moyenne potion de soin.", -1);
            ...
            {
                affichage_message(game, "Vous avez obtenue une moyenne potion de mana.", -1);
    ...
    else if(nb_enemie_combat == 2)
    {
        ...
        affichage_message(game, "Vous avez gagne %d xp.", nb_total_xp);
        character->xp += nb_total_xp;
        if(character->lvl == 50)affichage_message(game, "Vous etes au summum, vous ne pouvez plus gagnez d'xp.", -1);
        if(character->xp + character->tab_xp_max[character->lvl - 1] > character->tab_xp_max[character->lvl])
        {
            character->xp = character->xp + character->tab_xp_max[character->lvl - 1] - character->tab_xp_max[character->lvl];
            affichage_message(game, "Vous avez gagne un niveau.", -1);
